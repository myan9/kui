...
class StatusWatcher implements Abortable, Watcher {
  ...
  private readonly eventWatchers: Abortable[] = []
  ...
  private abortEventWatchers() {
    if (this.eventWatchers) {
      for (let idx = 0; idx < this.eventWatchers.length; idx++) {
        this.eventWatchers[idx].abort()
        this.eventWatchers[idx] = undefined
      }
    }
  }
  ...
  public init(pusher: WatchPusher) {
    ...
    this.resourcesToWaitFor
      .map((_, idx) => {
        ...
        eventWatcher.init()
        this.eventWatchers.push(eventWatcher)
