...
localDescribe('auth tests', function (this: common.ISuite) {
  ...
  it('should create an action foo2', () => cli.do(`create foo2 ${ROOT}/data/openwhisk/foo.js`, this.app)
    .then(cli.expectJustOK)
    .then(sidecar.expectOpen)
  ...
  it('should switch to the first namespace, using the CLI switch command', () => cli.do(`wsk auth switch ${ns1}`, this.app)
  ...
  it('should switch to the first namespace, using the CLI wsk auth add command', () => cli.do(`wsk auth add ${process.env.AUTH}`, this.app)
    .then(cli.expectOKWithCustom({ selector: '', expect: `namespace ${ns1}` })))

  // switch to the second namespace but don't save it
  it('should switch to the second namespace but don\'t save it locally, using the CLI wsk auth switch --no-save command', () => cli.do(`wsk auth switch ${ns2} --no-save`, this.app)
    .then(cli.expectOKWithCustom({ selector: '', expect: `namespace ${ns2}` })))
  ...
  it('should see the first namespace', () => cli.do('namespace current', this.app)
    .then(cli.expectOKWithCustom({ selector: '', expect: `${ns1}` })))

  // switch to the second namespace and save it
  it('should switch to the second namespace and save it locally, using the CLI wsk auth switch --save command', () => cli.do(`wsk auth switch ${ns2} --save`, this.app)
    .then(cli.expectOKWithCustom({ selector: '', expect: `namespace ${ns2}` })))
  ...
  it('should see the second namespace', () => cli.do('namespace current', this.app)
    .then(cli.expectOKWithCustom({ selector: '', expect: `${ns2}` })))

  // switch to the first namespace and save it
  it('should switch to the first namespace and save it locally, using the CLI wsk auth switch command', () => cli.do(`wsk auth switch ${ns1}`, this.app)
    .then(cli.expectOKWithCustom({ selector: '', expect: `namespace ${ns1}` })))
