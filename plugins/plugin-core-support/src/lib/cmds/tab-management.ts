...
export default function plugin(commandTree: Registrar) {
  ...
  >(
    ...
    async args => {
      ...
      const file = args.parsedOptions.file || args.parsedOptions.f
      if (file) {
        // caller wants to open a given snapshot by file in the new tab
        const filepath = expandHomeDir(file)
        const model = await loadSnapshotBuffer(args.REPL, filepath)
    ...
    },
    {
      ...
      usage: {
        optional: [
          ...
          { name: '--file', alias: '-f', docs: 'Snapshot file to display in the new tab' },
