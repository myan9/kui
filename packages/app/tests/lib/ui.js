  ...
  do: (cmd, app, noNewline = false) => {
    return app.client.waitForExist(selectors.CURRENT_PROMPT_BLOCK)
      ...
      .then(count => app.client.getValue(selectors.CURRENT_PROMPT)
        ...
        .then(() => { if (noNewline !== true) process.env.WEBPACK_TEST ? app.client.keys(keys.ENTER) : app.client.execute('repl.doEval()') }) // FIXME: repl is undefined in webpack
...
exports.sidecar = {
  ...
  expectSourceStruct: expectedJSON => app => app.client.getText(selectors.SIDECAR_ACTION_SOURCE)
    .then(exports.expectStruct(expectedJSON))
    .then(() => app),

  expectSourceSubset: expectedJSON => app => app.client.getText(selectors.SIDECAR_ACTION_SOURCE)
        ...
        if (nameMatches) {
          if (expectedPackageName) {
            return app.client.getText(selectors.SIDECAR_PACKAGE_NAME_TITLE)
              .then(name => expectSubstringMatchOnName ? name.search(new RegExp(expectedPackageName, 'i')) >= 0 : name.toLowerCase() === expectedPackageName.toLowerCase())
          } else {
            return true
          }
        }
