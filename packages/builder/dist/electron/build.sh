...
function win32 {
    ...
    if [ "$PLATFORM" == "all" ] || [ "$PLATFORM" == "win32" ] || [ "$PLATFORM" == "windows" ]; then
        ...
        (cd "$BUILDER_HOME/dist/electron" && NO_PRUNE=true node builders/electron.js "$CLIENT_HOME" "${PRODUCT_NAME}" win32 $ARCH $ICON_WIN32)
...
function linux {
    ...
    if [ "$PLATFORM" == "all" ] || [ "$PLATFORM" == "linux" ]; then
        ...
        (cd "$BUILDER_HOME/dist/electron" && NO_PRUNE=true node builders/electron.js "$CLIENT_HOME" "${PRODUCT_NAME}" linux $ARCH $ICON_LINUX)
...
function webpack {
    ...
    if [ -n "$KUI_HEADLESS_WEBPACK" ]; then
        ...
        MODE=${MODE-production} CLIENT_HOME="$CLIENT_HOME" KUI_BUILDDIR="$BUILDDIR" KUI_BUILDER_HOME="$BUILDER_HOME" npx --no-install webpack-cli --config ./node_modules/@kui-shell/webpack/headless-webpack.config.js --mode=${MODE-production} --config "$MODULE_HOME"/webpack/headless-webpack.config.js &
    fi

    # echo "Building electron bundles via webpack"                                                                     
    TARGET=electron-renderer MODE=${MODE-production} CLIENT_HOME="$CLIENT_HOME" KUI_BUILDDIR="$BUILDDIR" KUI_BUILDER_HOME="$BUILDER_HOME" npx --no-install webpack-cli --mode=${MODE-production} --config "$MODULE_HOME"/webpack/webpack.config.js
