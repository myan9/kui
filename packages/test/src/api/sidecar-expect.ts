...
import { promiseEach, TreeItem } from '@kui-shell/core'
...
export const tree = (expected: ExpectedTree[]) => async (res: AppAndCount) => {
  ...
  const testTree = async (nodes: ExpectedTree[]) => {
    await promiseEach(nodes, async node => {
      ...
      let idx = 0
      await res.app.client.waitUntil(async () => {
        const actualText = await res.app.client.getText(
        ...
        return actualText.includes(node.content)
      }, 20000)
