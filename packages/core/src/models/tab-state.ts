...
import { ResourceWatcher } from '../core/job'
...
export default class TabState {
  ...
  public captureJob(job: ResourceWatcher) {
    if (!this._jobs) {
      const maxJobs = theme.maxWatchersPerTab || 6
      this._jobs = new Array<ResourceWatcher>(maxJobs)
   ...
   *
   */
  public removeJob(job: ResourceWatcher) {
    if (this._jobs) {
