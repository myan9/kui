...
import { timeout, keys, waitTimeout, expectSubset, expectStruct } from './util'
import { selectors } from '@kui-shell/core/tests/lib/ui'
import { ISuite } from '@kui-shell/core/tests/lib/common'

export const expectOpen = async (app: Application) => {
...
export const expectClosed = async (app: Application) => {
...
export const doClose = async function(app: Application) {
  await expectOpen(app)
  await app.client.keys(keys.ESCAPE)
  await expectClosed(app)
}

/** helper method to open the sidecar */
export const doOpen = async function(app: Application) {
  await expectOpen(app)
  await app.client.keys(keys.ESCAPE)
  await expectOpen(app)
}
