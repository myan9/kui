...
localDescribe('List root-most non-erroring activations with $$!', function (this: common.ISuite) {
  ...
  it('should create an erroring sequence with let', () => cli.do(`let ${errorSeqName}=${goodActionName}->${errorActionName}`, this.app)
    .then(cli.expectJustOK)
    .then(sidecar.expectOpen)
    .then(sidecar.expectShowing(errorSeqName))
    .catch(common.oops(this)))
  it('should create a non-erroring sequence with let', () => cli.do(`let ${goodSeqName} = ${goodActionName} -> ${goodActionName}`, this.app)
    .then(cli.expectJustOK)
    .then(sidecar.expectOpen)
  ...
  // call await
  it('should await not-successful completion of the activation', () => cli.do(`$ await`, this.app)
    .then(cli.expectJustOK)
    .then(sidecar.expectOpen)
    .then(sidecar.expectShowing(errorSeqName))
    .then(() => this.app.client.getText(ui.selectors.SIDECAR_ACTIVATION_RESULT))
  ...
  it('should not list the non-erroring sequence activation', () => cli.do(`$$!`, this.app)
    .then(cli.expectOKWithCustom({ passthrough: true }))
    .then(N => this.app.client.elements(ui.selectors.LIST_RESULTS_BY_NAME_N(N)))
